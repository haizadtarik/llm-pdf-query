...
class uploadCSV:
    def __init__(self, uploaded_file, openai_key=None, st_encoder='sentence-transformers/gtr-t5-base'):
        if openai_key is not None:
            self.model = openAIencoder(openai_key)
        else:
            self.model = SentenceTransformer(st_encoder)
        self.qdrant_client = QdrantClient(host='localhost', port=6333)
        self.embeddings, self.chunks_dict = self.parse2doc(uploaded_file)
        self.create_collection(self.embeddings.shape[1])
        self.upload_vectors(self.embeddings,self.chunks_dict)

    def parse2doc(self, uploaded_file):
        """
        This function is used to parse csv file to documents
        ...
        if type(uploaded_file) == str:
            if '.parquet' in uploaded_file:
                self.collection_name = uploaded_file.split('/')[-1].split('.')[0]
        ...
        df["text"] = df.apply(lambda x: " ".join([f"{index}: {values};" for index,values in zip(x.index,x.values) if values is not None]),axis=1)
        chunks = df["text"].to_list()
